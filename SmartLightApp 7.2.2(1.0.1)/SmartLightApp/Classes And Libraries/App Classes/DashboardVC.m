...
-(void)setMainViewContent
{
    ...
    blueSegmentedControl = [[NYSegmentedControl alloc] initWithItems:@[@"Devices", @"Room"]];
    ...
    addFloatButton = [[VCFloatingActionButton alloc]initWithFrame:CGRectMake(DEVICE_WIDTH-70, DEVICE_HEIGHT-100, 60, 60) normalImage:[UIImage imageNamed:@"navbulb_icon.png"] andPressedImage:[UIImage imageNamed:@"navbulb_icon.png"] withScrollview:tblView];
...
-(void)btnMoreClick:(id)sender
{
    ...
    if (previousIndexPath == index)
    {
        previousIndexPath = [NSIndexPath indexPathForRow:-1 inSection:-1];
    }
    else
    {
        previousIndexPath = index;
    }
    [tblView reloadData];
}
-(void)btnHeaderClick
...
-(void)btnSettingsClick:(id)sender
{
    selectedIndexPathl = [NSIndexPath indexPathForRow:[sender tag] inSection:0];
    strDeviceID = [[sectionArr objectAtIndex:selectedIndexPathl.row] valueForKey:@"device_id"];
    globalDeviceHexId = [[sectionArr objectAtIndex:selectedIndexPathl.row] valueForKey:@"hex_device_id"];
    ...
    btn1.titleLabel.font = [UIFont fontWithName:CGRegular size:textSizes+2];
    ...
    btn3.titleLabel.font = [UIFont fontWithName:CGRegular size:textSizes];
    ...
    btn4.titleLabel.font = [UIFont fontWithName:CGRegular size:textSizes];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView  == tblView)
    {
        ...
        if (isForGroup)
        {
            CustomGroupCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            if (cell==nil)
            {
                cell = [[CustomGroupCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIdentifier];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath;
{
    if (tableView  == tblView)
    {
        ...
        if (isForGroup)
        {
            CustomGroupCell * tblCell = (CustomGroupCell *)[(UITableView *)tableView cellForRowAtIndexPath:indexPath];
...
- (void)orbSwitchToggled:(ORBSwitch *)switchObj withNewValue:(BOOL)newValue
{
    ...
    else
    {
        ...
        {
            CustomTableViewCell *cell = (CustomTableViewCell *)[tblView cellForRowAtIndexPath:tmpIndex];
            if (newValue)
            {
                cell.imgBulb.layer.shadowColor = [[UIColor grayColor] CGColor];
                cell.imgBulb.layer.shadowRadius = 4.0f;
...
- (void)setGradientBackgroundWithColors:(NSArray *)colors withSlider:(UISlider *)sliders
{
    CAGradientLayer *trackGradientLayer = [CAGradientLayer layer];
    CGRect frame = sliders.frame;
    ...
    trackGradientLayer.cornerRadius = 5;
    UIImage *trackImage = [[self imageFromLayer:trackGradientLayer] resizableImageWithCapInsets:UIEdgeInsetsMake(10, 10, 10, 10)];
    [sliders setMinimumTrackImage:trackImage forState:UIControlStateNormal];
    [sliders setMaximumTrackImage:trackImage forState:UIControlStateNormal];
}
