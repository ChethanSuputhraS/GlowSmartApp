...
-(void)NotifyforBridge:(NSNotification*)notification//Update peripheral
{
    ...
    //    {
    //        NSLog(@"NEW DEVICE ADDED === >>> %@",[[BLEManager sharedManager] nonConnectArr] );
    //
    //        [tblView reloadData];
    //        totalCounts= totalCounts + 1;
    //    }
    //            NSLog(@"NEW DEVICE ADDED === >>> %@",[[BLEManager sharedManager] foundDevices] );
...
-(void)ResetSuccessPopup:(NSNotification *)notify
{
    ...
    if (peripheral != nil)
    {
        ...
        {
            CBPeripheral *p = [[[[BLEManager sharedManager]foundDevices]objectAtIndex:i]valueForKey:@"peripheral"];
            if (p == peripheral)
            {
...
/*
 Do you want to factory reset the device blinking red? : Yes / No
 **/
